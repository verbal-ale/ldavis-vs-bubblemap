...
var LDAvis = function(to_select, data_or_file_name, hexmapData, distributionData, color1, color2) {
    ...
    var borderGroup, bubbleGroup, elements;
    var bubbleLabelMap = new Map();
            data_or_file_name["topic.order"].forEach((value, index) => {
                // Add the value and index+1 to the map
                bubbleLabelMap.set(value, index + 1);
              });
    ...
    function visualize(data, hexmapData) {
        
        var bubbleMapData = getBubbleMapData(hexmapData, distributionData);
        ...
        // var bubbleMap = mdsplot.append("g")
        //     .attr("id", "elements");
        //     // .attr("transform", "translate(" + margin.left + "," + 2 * margin.top + ")");
        ...
        */
         ...
         */
        // ------ INSERTERD CODE SNIPPED FROM HERE vvv ---------

        elements = mdsplot.append('g')
        .attr('id', 'elements')
        .attr('transform', `translate(${mdswidth/2},${mdsheight/2.5}) scale(1.6259)`); // scale adjusted after measuring difference with original LDAvis bubble size
        
        bubbleGroup = elements.append("g")
            .attr("class", "bubbles");
        borderGroup = elements.append("g")
        .attr("class", "contour");
        
        // let bubbleMapData = getBubbleMapData(hexmapData, distributionData);
        drawChart(bubbleMapData);
        
       // ====== SNIPPED CODE END HERE ==================
       svg.append("text")
            .text("Cluster Bubble Map ") // removed " (via multidimensional scaling)"
        ...
        d3.select("#" + barFreqsID)
            ...
            .attr("y", mdsheight + 10 + (6/2)*barguide.height + 5)
            .style("dominant-baseline", "middle");
            //.text("* - slide towards 0 if relevance within the topic is more important");
        d3.select("#" + barFreqsID)
            .append("a")
            .attr("xlink:href", "http://nlp.stanford.edu/events/illvi2014/papers/sievert-illvi2014.pdf")
            .attr("target", "_blank")
            ...
            .style("dominant-baseline", "middle");
            //.text("* - slide towards 1 if relevance within the collection is more important");
        ...
        var title = chart.append("text")
            ...
            .style("text-anchor", "middle")
            .style("font-size", "16px")
            .text("Top-" + R + " Terms Most Important for Topic Formation ");
        ...
        function init_forms(topicID, lambdaID, visID) {
            ...
            inputDiv.setAttribute("style", "display: none"); // to hide the input div
            ...
            var xx = d3.select("#" + lambdaZeroID)
                ...
                .text("Adjust balance of term relevance:");
        ...
        function drawChart(bubbleMapData) {
            let preSelectedTopic = null;
        
            let bubblesData = bubbleMapData[0],
                bordersData = bubbleMapData[1];
            ...
            borders.attr("d", function(arc){return arc.d;})
            ...
            bubblesEnter.append('circle')
                ...
                // .attr("stroke", "black")
                .attr("id", function(d) {
                    return (topicID + bubbleLabelMap.get(d.data.topicData.conceptId));
                })
                .on("mouseover", function(d) {
                    var old_topic = topicID + vis_state.topic;
                    if (vis_state.topic > 0 && old_topic != this.id) {
                ...
                .on("click", function(d) {
                    ...
                    // make sure topic input box value and fragment reflects clicked selection
                    let selected_topic = bubbleLabelMap.get(d.target.__data__.data.topicData.conceptId);
                    document.getElementById(topicID).value = vis_state.topic = selected_topic;
                    vis_state.lambda = 0.6;
                    state_save(true);
                    topic_on(this);
                })
                .on("mouseout", function(d) {
                    let selected_topic = bubbleLabelMap.get(d.target.__data__.data.topicData.conceptId);
                    if (vis_state.topic != selected_topic) topic_off(this);
                    if (vis_state.topic > 0) topic_on(document.getElementById(topicID + vis_state.topic));
                });
            
            
            bubblesEnter.append('g').classed('labels', true);
            bubblesEnter.append('g').classed('fake_labels', true);
            bubbles = bubbleGroup.selectAll('g.bubble')
                .classed('click', true);
            //     .classed('selected', d=>{return d.data.topicData.conceptId === selectedSuperTopic});
            let circles = bubbles.select('circle');
            let texts = bubbles.select('g.labels');
            let fake_texts = bubbles.select('g.fake_labels');
        
            bubbles.attr('transform', d=>`translate(${xScale(d.x)},${yScale(d.y)})`)
                .on('click', function(d){
            ...
            .data(d => {
                return [[d.data.topicData.conceptId.toString(), d.r]];
            });
        
            // Real labels are kept and made transperant so they can stil lbe used as IDs
            labels.enter().append("text").classed('label', true);
        
            labels = texts.selectAll("text.label");
            labels
                // .attr("font-size", (d, i) => {
                // return Math.max(d[1] / 3, 1);
                //     })
                .attr('dy', (d, i) => {
                    return d[1] / 9;
                })
                .text(d => {
                    return bubbleLabelMap.get(parseInt(d[0]));
                })
            ...
            // Initialize the array with string numbers from "1" to "20"
            var fakeLabelArray = [];
            for (var i = 1; i <= 20; i++) {
                fakeLabelArray.push(i.toString());
            }
            
            // Seed the random number generator with a fixed value
            var random = seedRandom(12345);
            
            // Shuffle the array deterministically
            fakeLabelArray.sort(function() {
                return random() - 0.5;
            });
                
            let fakeLabels = fake_texts.selectAll("text.fake_label")
            .data(d => {
                return [[d.data.topicData.conceptId.toString(), d.r]];
            });

            // Real labels are kept and made transperant so they can stil lbe used as IDs
            fakeLabels.enter().append("text").classed('fake_label', true);
    
            fakeLabels = fake_texts.selectAll("text.fake_label");
            fakeLabels
                // .attr("font-size", (d, i) => {
                // return Math.max(d[1] / 3, 1);
                //     })
                ...
                .text(d => {
                    return fakeLabelArray[d[0]-1];
                })
                .style('pointer-events', 'none');
        
                

            let svgTopSize = svg.node().getBBox(),
                w = svgTopSize.width,
                h = svgTopSize.height;
        ...
        function getBubbleMapData(hexmapData, distributionData){
            let hData = createHierarchyData(hexmapData,distributionData);
            let sizeScale = d3.scaleLinear()
                .domain([1, d3.max(getSizes(hData))])
                .range([5, 40]);
        ...
        function getPositionScale(circleData){
            let xMin = d3.min(circleData, d=>{return d.x-d.r}),
                xMax = d3.max(circleData, d=>{return d.x+d.r}),
                yMin = d3.min(circleData, d=>{return d.y-d.r}),
                yMax = d3.max(circleData, d=>{return d.y+d.r}),
                xRange = xMax-xMin,
                yRange = yMax-yMin;
        ...
        function topic_on(circle) {
            ...
            var Freq = Math.round(d.data.size/18846 * 10000)/100,
                topics = bubbleLabelMap.get(d.data.topicData.conceptId);
            ...
            d3.select("#" + barFreqsID)
                ...
                .style("text-anchor", "middle")
                .style("font-size", "16px")
                .text("Top-" + R + " Most Relevant Terms for Selected Topic " +  " (" + Freq + "% of documents)");
        ...
        function term_on(term) {
            ...
            // var bubblesSelectionOrder = [1, 14, 5, 10, 4, 2, 8, 13, 9, 6, 7, 3, 12, 0, 15, 17, 11, 16, 18, 19];
            // var reorderedRadius = new Array(radius.length).fill(0);

            // // Reorder the radius array based on the selection order
            // for (var i = 0; i < bubblesSelectionOrder.length; i++) {
            //     reorderedRadius[i] = radius[bubblesSelectionOrder[i]];
            // }

            // // Change size of bubbles according to the word's distribution over topics
            // d3.selectAll(to_select + " .dot")
            //     .data(reorderedRadius)
            //     .transition()
            //     .attr("r", function(d) {
            //         return (Math.sqrt(d*mdswidth*mdsheight*word_prop*0.25/Math.PI));
            //     });


            // // re-bind mdsData so we can handle multiple selection
            // d3.selectAll(to_select + " .dot")
            //     .data(bubbleMapData[0])
            ...
            console.log("funtion term_on disabled.");
        }
    ...
    if (typeof data_or_file_name === 'string')
        d3.json(data_or_file_name, function(error, data) {visualize(data, hexmapData);});
    else
        visualize(data_or_file_name, hexmapData);
