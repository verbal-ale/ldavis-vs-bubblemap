...
function createHierarchyData(mapData, distributionData){
    // make the hierarchy
    let hierarchy = makeHierarchy(mapData);
    // add the cluster and uncertainty information
    getClusters(hierarchy);
    // add the size information to the topics entries
    addSizes(hierarchy, distributionData);
...
function exploreNode(node, nTopics, topics, table){
    if(node >= nTopics){
        ...
        return {
            uncertainty: 0,
            leaf: false,
            children: [exploreNode(table[node-nTopics].node1, nTopics, topics, table),
...
// function addSizes(node, distribution){
//     if(node.leaf){
...
//         addSizes(node.children[0], distribution);
//         addSizes(node.children[1], distribution);
//     }
// }
...
function addSizes(node, distribution) {
    if (node.leaf) {
        // if the current node is a leaf node
        // then gets its distribution entry and sum the values in it
        // and set the node's size value to this sum
        let distributionEntry = distribution.filter(e => {
            return e.conceptId === node.topicData.conceptId;
        })[0];
        
        node.size = distributionEntry.topicData.numberOfDocuments;
    } else {
        // otherwise explore the children nodes
        addSizes(node.children[0], distribution);
        addSizes(node.children[1], distribution);
